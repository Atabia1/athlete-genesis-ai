...
export function useNetworkStatus(options: NetworkStatusOptions = {}) {
  ...
  // Always default to online=true to prevent false offline indicators
  ...
  const checkConnection = useCallback(async (): Promise<ConnectionCheckResult> => {
    ...
    try {
      ...
      // If the fetch fails, it could be due to various reasons
      // Don't immediately assume we're offline - check navigator.onLine first
      console.log('Connection check failed:', error);
      checkInProgressRef.current = false;

      // If navigator.onLine is true, we might have a network issue but not be fully offline
      // This helps prevent false offline indicators on 404 pages or other network errors
      if (navigator.onLine) {
        return {
          online: true,
          latency: null,
  ...
  useEffect(() => {
    // Initial connection check - but only if we're not on a 404 page
    // This helps prevent false offline indicators on error pages
    const isErrorPage = window.location.pathname.includes('non-existent-page') ||
    ...
    intervalRef.current = window.setInterval(() => {
      // Only perform check if navigator.onLine is true and we're not on an error page
      // This avoids unnecessary requests when we know we're offline
      const currentIsErrorPage = window.location.pathname.includes('non-existent-page') ||
                                document.title.includes('404') ||
                                document.title.includes('Not Found');

      if (navigator.onLine && !currentIsErrorPage) {
        updateConnectionStatus();
      }
